...
3. MULTIPLE_CHOICE: 보기(options)는 4지선다형을 기본으로 합니다. 정답('a')은 반드시 보기 중 하나와 정확히 일치해야 합니다.
    4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리된 단어들 중 순서)를 넣으세요.
      **[출제 기준 - 반드시 지킬 것]** 
      ...
      - **띄어쓰기 규칙 (매우 중요):** 문장 전체를 모든 단어 단위로 띄어 쓰지 마세요. **오직 정답(빈칸)이 되는 핵심 단어의 앞과 뒤에만 공백을 하나씩 추가**하여 해당 단어가 독립된 인덱스를 갖게 하세요. 문장의 나머지 부분은 일반적인 한국어 맞춤법에 맞춰 자연스럽게 띄어 쓰세요.
      - **성공 예시:** 정답 단어가 "지층"인 경우
        - (잘못된 예): "오랜 시간 쌓인 지층 은 여러 겹으로 보입니다." (모든 단어를 띄어씀)
        - (올바른 예): "오랜시간 쌓인 지층 은 여러겹으로 보입니다." (정답 '지층' 앞뒤에만 공백, 나머지는 붙여쓰거나 자연스럽게 유지) -> q: "오랜시간 쌓인 지층 은 여러겹으로 보입니다.", blanks: [2]
      - **성공 예시:** q: "지구 의 자전 으로 인해 낮과 밤이 생깁니다.", blanks: [0, 2] (정답 단어 '지구', '자전'만 앞뒤 공백으로 분리)
      - **주의:** 정답 단어에 조사가 붙어 있으면 안 됩니다. (예: '지층은' (X) -> '지층 은' (O))
