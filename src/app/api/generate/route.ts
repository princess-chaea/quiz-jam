...
3. MULTIPLE_CHOICE: 보기(options)는 4지선다형을 기본으로 합니다. 정답('a')은 반드시 보기 중 하나와 정확히 일치해야 합니다.
    4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리)를 넣으세요. 
      **[출제 기준 - 반드시 지킬 것]** 
      - 반드시 문장의 핵심 의미를 결정하는 **핵심 전문 용어, 개념어, 고유 명사(체언)**만을 빈칸으로 만드세요. 
      - **절대 금지:** '이라고', '한다', '있습니다', '입니다', '하고', '때문에' 와 같은 **조사, 어미, 보조 용언, 기능어**는 절대 단독으로 빈칸으로 만들지 마세요. 학생이 맥락으로 맞출 수 없는 무의미한 빈칸은 오답으로 처리됩니다.
      - **한국어 조사 처리:** 정답 단어가 조사(이/가, 은/는, 을/를, 에, 의 등)와 붙어있는 경우, 반드시 조사 앞에 공백을 두어 정답 단어만 빈칸이 되도록 분리하세요. (예: "빛이 있다" -> q: "빛 이 있다", blanks: [0] / "지층은" -> q: "지층 은", blanks: [0])
      - 정답 단어는 반드시 앞뒤로 띄어쓰기를 무조건 사용하여 정확히 해당 단어가 분리되도록 하세요.
      - **성공 예시:** q: "빛 의 굴절 이라고 한다", blanks: [0, 2] (정답: 빛, 굴절)
      - **실패 예시:** q: "빛의 굴절 이라고 한다", blanks: [3, 4] (정답: 이라고, 한다 -> **절대 금지**)
