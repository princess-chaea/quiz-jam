...
3. MULTIPLE_CHOICE: 보기(options)는 4지선다형을 기본으로 합니다. 정답('a')은 반드시 보기 중 하나와 정확히 일치해야 합니다.
    4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리)를 넣으세요. **반드시 핵심 용어, 개념, 명사(코어 키워드)를 빈칸으로 만드세요.** 동사(예: '추리할', '하는')나 일반적인 서술어는 지양하세요. 
      **[중요 - 한국어 조사 처리 (오직 'BLANK' 유형만 해당)]** 정답 단어가 조사(이/가, 은/는, 을/를, 에, 의 등)와 붙어있는 경우, 반드시 조사 앞에 공백을 두어 정답 단어만 빈칸이 되도록 분리하세요. (예: "빛이 있다" -> q: "빛 이 있다", blanks: [0] / "지층은" -> q: "지층 은", blanks: [0])
