...
[핵심 출제 원칙]
- 유형 균형: 선택된 유형(${typeLabel})이 여러 개인 경우, 전체 문항 수(${count}) 내에서 각 유형이 최대한 골고루 섞이도록 배분하세요.
...
4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리)를 넣으세요. 정답 단어는 반드시 앞뒤로 띄어쓰기를 하여 정확히 분리되도록 하세요. (예: q: '지층은 퇴적물 이 쌓여 층을 이룬 것이다.', blanks: [1], a: '퇴적물')
