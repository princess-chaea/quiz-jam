...
4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리)를 넣으세요. **반드시 핵심 용어, 개념, 명사(코어 키워드)를 빈칸으로 만드세요.** 동사(예: '추리할', '하는')나 일반적인 서술어는 지양하세요. 정답 단어는 반드시 앞뒤로 띄어쓰기를 무조건 사용하여 정확히 해당 단어가 분리되도록 하세요.
