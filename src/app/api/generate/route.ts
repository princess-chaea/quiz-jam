...
3. MULTIPLE_CHOICE: 보기(options)는 4지선다형을 기본으로 합니다. 정답('a')은 반드시 보기 중 하나와 정확히 일치해야 합니다.
    4. BLANK: 질문('q')에는 정답을 포함한 **완성된 전체 문장**을 넣으세요. 'BLANK', '( )', '____' 등의 표시를 **절대 사용하지 마세요.** 대신 'blanks' 배열에 해당 문장에서 빈칸으로 만들 단어의 인덱스(0부터 시작, 공백 기준 분리된 단어들 중 순서)를 넣으세요.
      **[출제 기준 - 반드시 지킬 것]** 
      - 반드시 문장의 핵심 의미를 결정하는 **핵심 전문 용어, 개념어, 고유 명사**만을 빈칸으로 만드세요. 
      - **띄어쓰기 및 조사 분리 규칙 (반드시 준수):** 한국어 문장에서 정답 단어에 조사가 붙어 있다면(예: '지구가', '지형은'), 반드시 **정답 단어와 조사를 공백으로 분리**하세요(예: '지구 가', '지형 은'). 
      - **정답('a')**은 반드시 조사를 제외한 **순수한 핵심 단어**만 기술하세요.
      - **성공 예시:** 정답 단어가 "지층"인 경우
        - (잘못된 예): q: "오랜 시간 쌓인 지층은 여러 겹입니다.", blanks: [3], a: "지층은" (조사가 포함됨)
        - (올바른 예): q: "오랜 시간 쌓인 지층 은 여러 겹으로 보입니다.", blanks: [3], a: "지층" (정답 앞뒤에 공백을 두어 조사를 분리함)
      - **성공 예시:** q: "지구 가 자전 을 하여 낮과 밤이 생깁니다.", blanks: [0, 2], a: "지구, 자전"
      - **주의:** 'SHORT_ANSWER'와 달리 'BLANK'는 질문 문장 내의 공백을 기준으로 인덱스를 매기므로, 빈칸으로 표시하고 싶은 단어는 반드시 앞뒤가 공백으로 분리되어야 합니다.
